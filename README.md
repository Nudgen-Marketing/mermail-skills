...
npx -y skills add Nudgen-Marketing/mermail-skills --skill mermail-procurement-agent -g -y --agent '*'
```
...
| `mermail-procurement-agent` | Acquire a named third-party service end to end on a frozen spend envelope: agent mailbox, emailed verification, one charge, then reconcile and file the receipt |
