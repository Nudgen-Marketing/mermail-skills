...
Official Mermail workflows for Codex, Claude Code, Cursor, and other Agent Skills-compatible clients. The plugin connects to the hosted Mermail MCP server for agent-inbox provisioning, verification mail, inbox management, email delivery, workspace administration, task triage, mailbox-agent workflows, inbox-hygiene cleanup, and Scheduling / GTM / Support / x402 agent personas.
...
| `mermail-inbox-hygiene-agent` | Audit senders, run approved reversible cleanup, and queue unsubscribes for review |
