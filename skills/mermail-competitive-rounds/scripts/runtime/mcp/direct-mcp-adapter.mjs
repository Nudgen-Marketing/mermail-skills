...
export class DirectMermailAdapter {
  constructor({ client, transport, writeEnabled = false, expectedToolSchemaHash = "0be3a178435a827e56e7be1b1d5f15215ed009a2425ee5c3f86fb98601f0bad8" } = {}) {
    this.client = client;
  ...
  static async connect({ writeEnabled = false } = {}) {
    const connection = await connectAuthenticated();
    return Object.freeze({
      adapter: new DirectMermailAdapter({ ...connection, writeEnabled }),
      connection,
    });
  }

  async preflight({ mailboxId, mailboxEmail, recipient, communicationRef, subject } = {}) {
    ...
    return Object.freeze({ allowed: true, current_state_digest: null, authority: "DIRECT_MCP_OAUTH_READONLY_PREFLIGHT", checks, endpoint: MCP_URL.href, writes: 0 });
  }
  ...
  async reconcile({ buyerMailboxId, supplierMailboxId, communicationRef, subject } = {}) {
    const observations = [];
    for (const [side, mailboxId] of [["buyer", buyerMailboxId], ["supplier_a", supplierMailboxId]]) {
      const result = await this.client.callTool({ name: "search_emails", arguments: { mailboxId, query: { query: communicationRef, subject, limit: 100, metadata_only: true } } });
      observations.push({ side, mailbox_id: mailboxId, result, candidates: candidates(decode(result)) });
    }
    return Object.freeze({ observations, read_only: true });
  }
