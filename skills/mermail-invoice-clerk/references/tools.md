...
| `paybox_request_transfer` | Exactly one call per approved invoice preview, with the previewed asset, chain, amount, and the policy destination. `chain` is a CAIP-2 id (Base = `eip155:8453`, Ethereum = `eip155:1`, Solana = `solana:mainnet`; the plain word `base` is rejected as `unsupported chain`), `token` is the ERC-20 contract or SPL mint (omit for the native asset), `amount` is in smallest units. PayBox owns approval, signing, and settlement. No `prepare_destructive_action` for `paybox_*`. Never retry an uncertain result; the one exception is a terminal `error` with `retryable: true` (the signing window expired before signing, nothing was spent), which allows one re-issue of the same previewed request. On `pending_signature`, PayBox may first require the user's agent signing key (generated once on PayBox's own setup page); the clerk never sees, requests, or pastes that key. |
