...
- One approved preview, one `paybox_request_transfer`, once. Never resubmit after a timeout or unknown result; reconcile once instead. The single exception is PayBox's own expiry signal — a terminal `error` with `retryable: true` (the signing window expired before anything was signed, nothing was spent) — which permits one re-issue of the same previewed request. An explicit new user request for the same invoice is otherwise a fresh action only after reconciliation shows no prior settlement.
