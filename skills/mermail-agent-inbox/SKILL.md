...
Read [resuming.md](references/resuming.md) when the user returns after a pause,
the client restarts, or an inbox wait needs a handoff. Save only an optional,
user-scoped checkpoint of the original expected-message tuple and baseline IDs.
Never save an OTP, link, body, token, or approval in a checkpoint.

Reconfirm the active user task and rediscover the credential-bound workspace and
exact mailbox before reading. Keep the original trigger time; do not replace it
with the return time. Search may be partial: exhaust the bounded relevant pages
or report `incomplete` before claiming one unique match. Re-read any selected
message and its scan gate. A saved `validated` state does not survive a pause.
Neither newest-first sorting nor a newer arrival resolves multiple matches.

Use the optional dependency-free [metadata helper](scripts/resume-inbox.mjs)
to validate a checkpoint and classify already normalized metadata. It does not
call Mermail, interpret bodies, authenticate senders, or authorize actions.
Follow the remaining live-content and expiry checks in the reference before
reporting a handoff; a current metadata match is not a usable sign-in token.
...
- "I am back after ten hours. Resume the same signup, tell me what is still pending, and do not use an old code."
