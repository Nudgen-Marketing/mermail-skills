...
4. Wait with bounded repeats of a metadata-only `search_emails` filtered by the expected tuple (`from`, subject marker, `to` = probe address, `date_start`, `include_held: true`) until the deadline. Reaching the deadline is not proof the provider failed — Mermail may hold mail briefly for triage. Report `probe_timeout` with that caveat; never re-trigger a send or extend the deadline silently.
