...
2. `SKILL.md` links every reference you ship (`tools`, `security`, `workflows`, `examples`, `harness`, `harness-compat`, `harness-audit`, `ci-failures`, `discussion-triage`, `discussion-triage-fixtures`, `pr-salvage`, `pr-salvage-corpus`, `analyzer-fixtures`, `evaluator-rag-fixtures`).
