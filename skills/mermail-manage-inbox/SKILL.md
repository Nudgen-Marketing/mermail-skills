...
3. Discover candidates with bounded `list_emails` or `search_emails` calls. Pass `query` as a native JSON object and never stringify it. Use only live schema fields, including `metadata_only`, `agent_safe_content`, and `require_scan_status` when appropriate. For `list_emails`, use `sortColumn: "date"` plus `sortDirection: "DESC"` for newest-first ordering; never invent `sort: "date_desc"` or assume those sort fields apply to `search_emails`. Normalize list response shapes and set a page/message budget as described in [tools.md](references/tools.md); retain the selected scope and report remaining results as yes, no, or unknown.
4. Select exact email ids before reading bodies or writing. Use `get_email` for one message, `get_email_context` for a bounded sanitized page around one selected message, and `get_thread` only when its broader thread representation is actually needed. Page inside the same scope before widening filters. A successful response with `content_omitted: true` supplies metadata, not a body read; `content_truncated: true` supports only a partial summary.
...
9. Before deletion, read the exact resource and determine its current folder and delivery status. Follow the regular-draft, scheduled-draft, Trash, and non-draft branches in [workflows.md](references/workflows.md); never describe every `delete_email` call as the same effect. Deleting a custom folder moves its filed messages to Trash before removing the folder.
...
- Delete only deletable custom folders. Do not call `delete_folder` for Inbox, Sent, Draft, Trash, Scheduled, or another non-deletable system folder. Preview the count of messages that will move to Trash, and report the returned `movedToTrashCount` after deletion.
...
- State filters, page/limit, sort order, read budget, and whether another page or opaque cursor exists for bounded reads. Preserve these details in saved reports alongside selected ids, remaining status, omissions, truncations, and the exact page or message responsible for a read error.
- Distinguish complete coverage of the selected scope from partial or unknown coverage. No matches under an unread or other restricted filter does not establish that the mailbox or an underlying process has no issues.
