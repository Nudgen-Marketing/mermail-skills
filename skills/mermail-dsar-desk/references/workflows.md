...
4. `move_email` into the `identity-pending` folder (`list_folders` → `create_folder` only if missing). Report the folder only if the move succeeded.
...
4. On confirmation: `move_email` from `identity-pending` to `in-progress`, and proceed.
...
5. `move_email` to `fulfilled` (or leave in `in-progress` if partial and user says so). Summarize without dumping raw PII.

## Reject or escalate

1. Rejection (e.g. unverified, out of scope, abusive): draft notice → exact preview → approved send → `move_email` to `rejected`.
2. Escalate: `forward_email` to the named privacy owner (or draft to them) → `move_email` to `escalated`. Say why (ambiguous identity, legal complexity, conflicting systems)—without claiming legal conclusions.
