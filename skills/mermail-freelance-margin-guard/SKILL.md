...
The test run must validate 17 skills, 71 business tools, and 57 dedicated Margin Guard checks. The packet must report `scope_change_detected`, 26–33 added hours, a 487.5–618.75 USD requested-deadline total, exactly three client options, and deterministic evidence and packet digests. This path uses only the bundled synthetic fixture, performs no network request, sends no email, and exposes no private project data.
