...
When interacting with the Mermail MCP server, the agent executes following workflow:
```json
{
...
###: Automatically triages, classifies, and tags incoming Mermail inbox emails, identifying high-priority invoices, OTP verification codes, and security alerts.
...
2. Check for invoice indicators: keywords like "Invoice", "Receipt", "Billing", "Payment Confirmation", or currency signs ($, €, £).
   - If matched, tag as `INVOICE_RECEIPT
