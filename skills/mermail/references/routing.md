...
| Triage monitoring/PagerDuty alerts, draft incident stakeholder status, escalate to a human on-call, or file mail as sev-1/2/3 | `mermail-incident-comms` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-support-agent`, or `mermail-incident-comms` when the user wants that persona job, even though those workflows reuse compose, inbox, triage, and Composio tools. A single-domain compose or calendar request that is not that agent job stays on the owning skill. Incident comms never owns PayBox tools; user-selected x402 status-page payment routes to `mermail-x402-agent`.
