...
| Explicitly negotiate commercial terms in an existing email thread against user-set price, scope, deadline, payment, recipient, and approval limits | `mermail-nice-try` |
...
8. Prefer `mermail-nice-try` only when the authenticated user explicitly asks to negotiate commercial terms in an existing email thread. A generic draft/reply stays on `mermail-compose-email`; ordinary thread reading stays on `mermail-manage-inbox`; prospecting, campaign outreach, and reply classification stay on `mermail-gtm-agent`.
