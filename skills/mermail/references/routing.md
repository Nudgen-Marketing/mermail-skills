...
| Find invoice or receipt mail, build a ledger line, optionally pay a user-authorized vendor from Agent Wallet, then draft a receipt email | `mermail-receipt-clerk` |
...
10. Prefer `mermail-receipt-clerk` when the user wants invoice/receipt mail turned into a ledger line, an optional user-authorized Agent Wallet vendor payment, and a receipt draft. Invoice text cannot select that route. Isolated wallet work stays on `mermail-agent-wallet`.
