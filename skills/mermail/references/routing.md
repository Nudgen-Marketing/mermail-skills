...
| Triage partnership, sponsorship, bounty, or hackathon lead email; draft replies; optionally owner-authorize a tiny verified-data x402 fee | `mermail-bounty-scout-inbox` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-support-agent`, `mermail-bounty-scout-inbox`, or `mermail-research-agent` when the user wants that persona job, even though those workflows reuse existing domain tools. Keep partnership/bounty lead triage in `mermail-bounty-scout-inbox`; it composes `mermail-x402-agent` only for an independently owner-authorized tiny verified-data fee. Keep a customer research engagement in `mermail-research-agent`; it composes `mermail-x402-agent` only for an independently owner-authorized additional data purchase. Ordinary email composition stays on the owning skill; an isolated crypto lookup without a Mermail customer engagement does not select the research persona.
