...
| Maintain a relationship ledger from mailbox evidence, surface stale threads or open loops, or draft re-engagement follow-ups for approval | `mermail-contact-keeper` |
...
10. Prefer `mermail-contact-keeper` when the user wants a contact ledger, open-loop tracking, staleness flags, or drafted re-engagement nudges grounded in mailbox evidence. Generic historical search stays on `mermail-manage-inbox`; a one-off composition stays on `mermail-compose-email`. This persona does not own those tools.
