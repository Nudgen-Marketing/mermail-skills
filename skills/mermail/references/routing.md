...
| Audit recurring subscriptions, trials, renewals, or price increases from receipts already stored in a mailbox, or draft a cancellation request from that evidence | `mermail-subscription-auditor` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-support-agent`, or `mermail-subscription-auditor` when the user wants that persona job, even though those workflows reuse compose, inbox, triage, and Composio tools. A single-domain compose or calendar request that is not that agent job stays on the owning skill. Keep one live expected receipt or verification mail on `mermail-agent-inbox`; the subscription auditor works over stored historical receipts.
