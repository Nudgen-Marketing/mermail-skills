...
| Take inbound paid briefs from a Mermail inbox, verify settlement independently, then deliver in-thread | `mermail-paid-intake` |
...
10. Prefer `mermail-paid-intake` when inbound email is a paid brief that must be verified independently, then delivered in-thread. Isolated x402 pay-then-continue stays on `mermail-x402-agent`. Isolated wallet inspect stays on `mermail-agent-wallet`. Unpaid support stays on `mermail-support-agent`.
