...
| Build a bounded receipt, invoice, or vendor-spend digest and save it as a draft | `mermail-receipt-digest-agent` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-support-agent`, or `mermail-receipt-digest-agent` when the user wants that persona job, even though those workflows reuse compose, inbox, triage, and Composio tools. A single-domain compose, search, or calendar request that is not that agent job stays on the owning skill. A receipt digest is more than a search: it must draft a briefing, not only list invoice mail.
