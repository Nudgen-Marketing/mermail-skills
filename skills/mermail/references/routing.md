...
| Turn a designated mailbox into a local-inference work queue: poll `[WORK]` task-card emails, run them on the user's own OpenAI-compatible local endpoint, and reply results in-thread | `mermail-local-worker` |
