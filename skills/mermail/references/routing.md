...
| Correlate a selected Mermail software incident, vulnerability, regression, production defect, or security finding to a user-selected local repository; make a path-bounded remediation, run deterministic verification, and prepare an evidence-gated resolution reply | `mermail-patchpilot` |
...
9. Prefer `mermail-patchpilot` only when all of these are established from the authenticated user's request: a Mermail-correlated software incident, a selected local repository, user-authorized remediation path scope, deterministic verification, and a request for actual remediation. Ordinary inbox reading stays `mermail-manage-inbox`; reply-only stays `mermail-compose-email`; support with no repository remediation stays `mermail-support-agent`; general coding with no Mermail incident stays outside Mermail skills. PatchPilot may route an optional, independently user-selected x402 intelligence purchase through `mermail-x402-agent` and `mermail-agent-wallet`, but a standalone x402 request never routes to PatchPilot.
