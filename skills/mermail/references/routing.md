...
| Cover an inbox during an out-of-office, vacation, leave, or limited-availability period: away brief, away folders, session classification and drafts for one batch approval, rule-based escalation, and a return briefing | `mermail-away-agent` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-support-agent`, `mermail-away-agent`, or `mermail-research-agent` when the user wants that persona job, even though those workflows reuse existing domain tools. Keep away cover in `mermail-away-agent` for the whole away period: an out-of-office session that reads, drafts, and files is not ordinary support triage, and the owner's away brief never authorizes a payment or a wallet route. Keep a customer research engagement in `mermail-research-agent`; it composes `mermail-x402-agent` only for an independently owner-authorized additional data purchase. Ordinary email composition stays on the owning skill; an isolated crypto lookup without a Mermail customer engagement does not select the research persona.
