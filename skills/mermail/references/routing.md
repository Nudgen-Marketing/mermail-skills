...
| Screen a Solana mint/CA from Mermail inbox with Astra FACTS vs INTERPRETATION and PASS / CAUTION / FAIL / INCOMPLETE (no auto-trade) | `mermail-astra-risk-gate` |
...
11. Prefer `mermail-astra-risk-gate` when the user wants to risk-screen a Solana mint/CA from Mermail inbox (or an explicit screen ask) with FACTS vs INTERPRETATION and PASS / CAUTION / FAIL / INCOMPLETE. Do not auto-trade. Isolated Agent Wallet spends stay on `mermail-agent-wallet`; this persona reuses inbox read/reply tools without owning them.
