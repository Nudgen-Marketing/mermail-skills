...
| Take over one selected conversation or prepare a read-only handoff of decisions, changed proposals, open questions, and next actions with message evidence | `mermail-thread-handoff` |
...
4. Route later historical receipt search, cleanup, organization, attachment, folder, or custom-label-definition work to `mermail-manage-inbox`. Prefer `mermail-thread-handoff` only for an explicit conversation takeover or evidence-linked handoff brief; ordinary summaries remain inbox management. A handoff request does not authorize forwarding or any mailbox write.
