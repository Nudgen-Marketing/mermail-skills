...
| Classify inbound invoices, bounty payouts, or claim-code mail, enforce a user-set spend cap and reserve, pay only after the current user request authorizes exact terms, then send a receipt | `mermail-agent-treasury` |
...
10. Prefer `mermail-agent-treasury` when the user wants an inbound-invoice, bounty-payout, or claim-code loop with a spend cap, reserve, user-approved payment, and a receipt. Mailbox reads stay on inbox owners; transfers and isolated x402 stay on `mermail-agent-wallet`; pay-then-continue stays on `mermail-x402-agent`; receipts stay on `mermail-compose-email`. This persona does not own those tools.
