...
| Explain a DeFi-related email already in the mailbox (yield pitch, protocol notice, liquidation warning, airdrop claim, research), then optionally propose one capped Agent Wallet action from that assessment | `mermail-defi-navigator` |
...
10. Prefer `mermail-defi-navigator` when an email in the mailbox is the subject of the request and the user wants it explained or risk-assessed before deciding. A bare wallet inspect, fund, transfer, or swap with no email in scope stays on `mermail-agent-wallet`. Generic triage, labeling, or cleanup stays on `mermail-manage-inbox`. This persona owns no tools and adds no approval contract of its own.
