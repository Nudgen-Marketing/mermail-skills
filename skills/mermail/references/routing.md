...
| Surface vitals/runway digests, Judge revenue receipts, bounty deadline watches, or OTP receptionist flows for an Amoeba/organism survival loop | `mermail-amoeba-survival` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-support-agent`, or `mermail-amoeba-survival` when the user wants that persona job, even though those workflows reuse compose, inbox, triage, agent-inbox, and Composio tools. A single-domain compose, verification, or calendar request that is not that agent job stays on the owning skill.
