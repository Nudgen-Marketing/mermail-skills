...
| Audit a mailbox for phishing, spoofing, or prompt-injection threats, verify sender authentication, or quarantine confirmed phishing | `mermail-security-audit` |
...
7. Use `mermail-composio` only for explicit third-party integration intent. Keep Gmail and Outlook email work inside Mermail rather than Composio.
 Use `mermail-security-audit` for explicit security-sweep, phishing-verdict, or quarantine intent, even though it reuses mailbox read, label, move, and draft tools owned by other skills.
