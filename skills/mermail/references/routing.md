...
| Protect freelance project margin by comparing approved scope with later requests, tracking revision budget or attributable delay, pricing owner-approved additions, preparing negotiation options, or verifying that one selected change order received its exact owner-approved public settlement | `mermail-freelance-margin-guard` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-support-agent`, `mermail-research-agent`, `mermail-freelance-margin-guard`, or `mermail-xstocks-desk` when the user wants that persona job, even though those workflows reuse existing domain tools. Keep a customer research engagement in `mermail-research-agent`; it composes `mermail-x402-agent` only for an independently owner-authorized additional data purchase. Keep freelance scope, revision-budget, schedule, pricing, negotiation-option analysis, and read-only proof of an exact prepaid change order in `mermail-freelance-margin-guard`; route actual wallet inspection or money movement to `mermail-agent-wallet`. Ordinary single-domain email, inbox, calendar, or crypto work that is not one of those persona jobs stays on the owning skill; an isolated crypto lookup without a Mermail customer engagement does not select the research persona.
