...
| Digest unread newsletters and digests from an agent Mermail inbox into a curated roundup email sent to the user | `mermail-newsletter-monitor` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-newsletter-monitor`, `mermail-gtm-agent`, or `mermail-support-agent` when the user wants that persona job, even though those workflows reuse compose, inbox, triage, and Composio tools. A single-domain compose, calendar, or digest request that is not that agent job stays on the owning skill.
