...
| Compare vendor terms, privacy, data-processing, security, or service-policy change notices | `mermail-policy-change-radar` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-support-agent`, or `mermail-policy-change-radar` when the user wants that focused job, even though those workflows reuse compose, inbox, triage, and Composio tools. A single-domain compose or calendar request that is not that focused job stays on the owning skill.
