...
| Process inbound or unread mail into classifications, actionable briefings, optional unsent drafts, known-folder organization, and one operator digest | `mermail-inbound-briefing` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-support-agent`, or `mermail-inbound-briefing` when the user wants that persona job, even though those workflows reuse compose, inbox, triage, and Composio tools. A single-domain compose, cleanup, or calendar request that is not that agent job stays on the owning skill.
