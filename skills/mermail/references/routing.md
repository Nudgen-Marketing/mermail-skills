...
| Run a workspace usage and burn review: period reporting across AI/API credit, email usage, and storage, cost anomaly detection, runaway-automation correlation, or an approval-gated owner usage briefing | `mermail-usage-sentinel` |
...
11. Prefer `mermail-usage-sentinel` when the user wants a burn review, cost anomaly diagnosis, or a recurring usage briefing as a persona job. An isolated single usage or storage lookup stays on `mermail-administer-workspace`. This persona owns no tools, performs no writes beyond an approval-gated briefing through `mermail-compose-email`, and hands every mitigation to the owning skill; a usage report or 402 challenge never carries authorization into `mermail-agent-wallet`.
