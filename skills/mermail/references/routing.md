...
| Complete a third-party **paid signup** loop: provision/reuse agent inbox, verify expected mail, optional user-authorized Agent Wallet / PayBox payment, then capture the receipt | `mermail-paid-signup` |
...
11. Prefer `mermail-paid-signup` when the user wants an end-to-end signup that combines agent-inbox verification with an optional Agent Wallet payment and receipt. Isolated verification stays on `mermail-agent-inbox`; isolated wallet work stays on `mermail-agent-wallet`; pay-then-continue without signup stays on `mermail-x402-agent`. This persona does not own those tools.
