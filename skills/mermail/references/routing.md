...
| Watch Superteam Earn, sponsor POCs, OTP/login codes, or bounty replies; classify action / ignore / escalate-to-human; draft builder-voice replies; optional read-only PayBox | `mermail-earn-ops` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-support-agent`, or `mermail-research-agent` when the user wants that persona job, even though those workflows reuse existing domain tools. Prefer `mermail-earn-ops` when the user wants Superteam Earn / bounty-ops inbox work (classify, draft-only, OTP surface, escalate money). It reuses manage-inbox / compose / wallet-read tools and does not own them. Keep a customer research engagement in `mermail-research-agent`; it composes `mermail-x402-agent` only for an independently owner-authorized additional data purchase. Ordinary email composition stays on the owning skill; an isolated crypto lookup without a Mermail customer engagement does not select the research persona.
