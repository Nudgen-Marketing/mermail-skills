...
| Audit vendor invoices, enforce anti-address-poisoning allowlists, check deliverable proofs, verify solvency, stage PayBox transfers, and record Solscan payout receipts | `mermail-treasury-guardian` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-support-agent`, `mermail-research-agent`, `mermail-treasury-guardian`, or `mermail-xstocks-desk` when the user wants that persona job, even though those workflows reuse existing domain tools. Keep a customer research engagement in `mermail-research-agent`; it composes `mermail-x402-agent` only for an independently owner-authorized additional data purchase. Ordinary email composition stays on the owning skill; an isolated crypto lookup without a Mermail customer engagement does not select the research persona.
...
11. Prefer `mermail-treasury-guardian` when the user wants vendor invoice auditing, anti-address-poisoning allowlist checks, deliverable proof verification, treasury solvency checks, and staged PayBox transfers with Console signing handoffs. Isolated transfers or wallet queries stay on `mermail-agent-wallet`; isolated email management stays on `mermail-manage-inbox`. This persona does not own those tools.
