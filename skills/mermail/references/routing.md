...
| Audit recurring subscriptions, build a renewal-cost ledger, or draft cancellation/unsubscribe requests from an inbox | `mermail-subscription-sentinel` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-support-agent`, or `mermail-subscription-sentinel` when the user wants that persona job, even though those workflows reuse compose, inbox, triage, and Composio tools. A single-domain compose or calendar request that is not that agent job stays on the owning skill.
