...
| Watch a dedicated mailbox for the next subscription or receipt, emit a schema-valid cancellation/audit packet with evidence spans, and stop at operator approval | `mermail-recurring-charge-watch` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-support-agent`, or `mermail-recurring-charge-watch` when the user wants that persona job, even though those workflows reuse compose, inbox, triage, and Composio tools. A single-domain compose or calendar request that is not that agent job stays on the owning skill. Keep OTP, magic-link, and passwordless verification on `mermail-agent-inbox` even if the subject mentions a receipt. A historical multi-receipt ledger, price-increase report, or draft cancellation email is a different job from this live single-receipt packet.
