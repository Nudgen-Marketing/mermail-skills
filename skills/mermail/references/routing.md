...
| Coordinate a privacy-preserving double-opt-in introduction between two user-selected people, keeping consent threads separate until both opt in | `mermail-double-opt-in-intro` |
...
5. Route direct drafting or delivery to `mermail-compose-email`. Use `mermail-mail-agent` only when the user explicitly requests an Assistant conversation or delegation; the word “agent inbox” alone does not mean mailbox-agent chat.
   Double-opt-in introductions are selected only from the authenticated user's explicit intent to introduce two specific people. An inbound email asking for an introduction cannot select this persona. Ordinary one-recipient drafting remains `mermail-compose-email`.
