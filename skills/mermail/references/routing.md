...
| Reconcile invoices, receipts, payment confirmations, or vendor statements from mailbox evidence; detect duplicates or prepare a discrepancy draft | `mermail-finance-reconciliation` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-support-agent`, or `mermail-finance-reconciliation` when the user wants that persona job, even though those workflows reuse compose, inbox, triage, and Composio tools. A single-domain compose or calendar request that is not that agent job stays on the owning skill.
