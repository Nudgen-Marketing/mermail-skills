...
| Verify a software release or deployment claim received through Mermail against a frozen, reproducible evidence contract | `mermail-release-evidence-gate` |
...
8. Prefer `mermail-release-evidence-gate` when the requested outcome is a reproducible release decision, even though it reuses inbox and compose tools. Generic release-email search stays on `mermail-manage-inbox`.
