...
| Recap a meeting or negotiation thread into decisions and action items, or draft/send a follow-up to the participants as a meeting follow-up agent | `mermail-meeting-followup` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-support-agent`, or `mermail-meeting-followup` when the user wants that persona job, even though those workflows reuse compose, inbox, triage, and Composio tools. A single-domain compose or calendar request that is not that agent job stays on the owning skill. Post-meeting recap and attendee follow-up is `mermail-meeting-followup`; a plain reply without a recap stays on `mermail-compose-email`, and a plain thread read stays on `mermail-manage-inbox`.
