...
| Answer an inbound enterprise security, DDQ, or vendor-risk questionnaire from a user-approved evidence pack | `mermail-security-questionnaire-desk` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-security-questionnaire-desk`, or `mermail-support-agent` when the user wants that persona job, even though those workflows reuse compose, inbox, triage, and Composio tools. A single-domain compose or calendar request that is not that agent job stays on the owning skill.
