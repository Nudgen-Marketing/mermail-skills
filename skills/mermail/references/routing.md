...
| Build a bounded spend ledger from receipt or invoice emails, optionally compared to a budget | `mermail-receipt-ledger` |
...
4. Route a spend ledger or receipt/invoice rollup with optional budget comparison to `mermail-receipt-ledger`. Route later historical receipt search, cleanup, organization, attachment, folder, or custom-label-definition work to `mermail-manage-inbox`.
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-support-agent`, or `mermail-receipt-ledger` when the user wants that persona job, even though those workflows reuse compose, inbox, triage, and Composio tools. A single-domain compose or calendar request that is not that agent job stays on the owning skill.
