...
| Turn receipts, invoices, or billing mail into verified expense records, reconcile spend, or flag duplicates and overcharges | `mermail-expense-agent` |
...
9. Prefer `mermail-expense-agent` when the user wants receipt or invoice extraction, an expense log, reconciliation, or duplicate/overcharge flags, even though it reuses manage-inbox and compose tools. A plain search, move, or folder request without the expense job stays on the owning skill.
