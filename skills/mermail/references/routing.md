...
| Detect material public dependency/docs/MCP/install-path breaks and file evidence-backed exception digests into Mermail (inbox-only; no wallet) | `mermail-integration-tripwire` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-support-agent`, `mermail-research-agent`, `mermail-integration-tripwire`, or `mermail-xstocks-desk` when the user wants that persona job, even though those workflows reuse existing domain tools. Keep a customer research engagement in `mermail-research-agent`; it composes `mermail-x402-agent` only for an independently owner-authorized additional data purchase. Ordinary email composition stays on the owning skill; an isolated crypto lookup without a Mermail customer engagement does not select the research persona. Prefer `mermail-integration-tripwire` for public dependency fingerprinting and exception digests; ordinary one-off compose stays on `mermail-compose-email`, and PayBox stays out of tripwire v1.
