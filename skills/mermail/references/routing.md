...
| Run a grant milestone desk: evaluate deliverable submissions, PR/artifact criteria checks, standing grant budget caps in Solana USDC, PayBox milestone transfer proposals, and audit receipt emails | `mermail-grant-milestone-desk` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-support-agent`, `mermail-research-agent`, `mermail-xstocks-desk`, or `mermail-grant-milestone-desk` when the user wants that persona job, even though those workflows reuse existing domain tools. Keep a customer research engagement in `mermail-research-agent`; it composes `mermail-x402-agent` only for an independently owner-authorized additional data purchase. Ordinary email composition stays on the owning skill; an isolated crypto lookup without a Mermail customer engagement does not select the research persona.
...
11. Prefer `mermail-grant-milestone-desk` when the user wants grant deliverable review, milestone criteria evaluation, PayBox milestone transfer proposals, or grant settlement receipt emails. Isolated transfers stay on `mermail-agent-wallet`; isolated compose stays on `mermail-compose-email`. This persona does not own those tools.
