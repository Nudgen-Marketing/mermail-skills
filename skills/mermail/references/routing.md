...
| Require a designated human to unlock an exact frozen send, invite, or PayBox spend via Mermail approval-request email and matching reply code before execution | `mermail-approval-gate` |
...
11. Prefer `mermail-approval-gate` when the operator explicitly wants a second-person email approval code gate before an external effect. Ordinary compose, invite, or wallet actions without that gate stay on their owning skills. The gate does not own tools and must not let the approval reply rewrite frozen args.
