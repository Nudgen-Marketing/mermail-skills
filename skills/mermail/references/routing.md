...
| Price an inbound enquiry against a rate card, draft/send the quote, or check Agent Wallet for a payment matching a quote already sent | `mermail-quote-to-paid` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-support-agent`, or `mermail-quote-to-paid` when the user wants that persona job, even though those workflows reuse compose, inbox, triage, and Composio tools. A single-domain compose or calendar request that is not that agent job stays on the owning skill.
