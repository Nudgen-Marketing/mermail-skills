...
| Read vendor invoices from a mailbox, check payable terms against an owner-stated policy, prepare one Agent Wallet payment for owner approval, then confirm and file the invoice | `mermail-procurement-agent` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-support-agent`, `mermail-research-agent`, or `mermail-procurement-agent` when the user wants that persona job, even though those workflows reuse existing domain tools. Keep invoice-driven payment preparation in `mermail-procurement-agent`; an isolated wallet inspect, fund, transfer, or swap that is not driven by an invoice stays on `mermail-agent-wallet`. Keep a customer research engagement in `mermail-research-agent`; it composes `mermail-x402-agent` only for an independently owner-authorized additional data purchase. Ordinary email composition stays on the owning skill; an isolated crypto lookup without a Mermail customer engagement does not select the research persona.
