...
| Track invoices, receipts, or billing notices, build a spend ledger, flag due dates or anomalies, or draft vendor reminders from billing mail | `mermail-receipts-agent` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-support-agent`, or `mermail-receipts-agent` when the user wants that persona job, even though those workflows reuse compose, inbox, triage, and Composio tools. A single-domain compose or calendar request that is not that agent job stays on the owning skill. Payment intent surfacing from billing mail routes to `mermail-agent-wallet`; the receipts persona never pays.
