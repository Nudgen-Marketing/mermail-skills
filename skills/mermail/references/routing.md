...
| Independently verify an inbound email's payment claim on-chain (correct recipient, amount, token contract, confirmed/finalized status, no replay) before releasing a deliverable or booking income | `mermail-proof-of-payment` |
...
11. Prefer `mermail-proof-of-payment` when the user explicitly requests independent on-chain verification of an inbound email's payment claim before releasing a deliverable or booking income. The skill reads the email via `mermail-manage-inbox` tools and performs its own read-only RPC verification — it does not own MCP tools.
