...
| Scan a mailbox for invoices and payment requests, validate exact terms, run an approval-gated Agent Wallet payment, then confirm and file the thread | `mermail-billing-desk` |
...
10. Prefer `mermail-billing-desk` when the job is invoice/payment-request triage or approval-gated bill payment from a mailbox. Isolated wallet inspection, swaps, or x402 stays on its owning skill; this workflow keeps every PayBox contract on `mermail-agent-wallet`.
