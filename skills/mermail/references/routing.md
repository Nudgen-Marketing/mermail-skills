...
| Broker a double-opt-in warm introduction (A asks to meet B; confirmation drafts plus a held intro that is never sent until both confirm) | `mermail-intro-broker` |
...
8. Prefer `mermail-scheduling-agent`, `mermail-gtm-agent`, `mermail-support-agent`, or `mermail-intro-broker` when the user wants that persona job, even though those workflows reuse compose, inbox, triage, and Composio tools. A single-domain compose or calendar request that is not that agent job stays on the owning skill. Warm introductions that require double opt-in belong on `mermail-intro-broker`, not GTM outbound and not a one-off compose.
