...
| `send_email` | mermail-compose-email | Mail the handoff note to the agent's own address (`to` = the mailbox address, `from` = the mailbox address). Self-addressed mail is stored in the Sent folder, not the inbox; search-based resume, list, and clear are unaffected. |
