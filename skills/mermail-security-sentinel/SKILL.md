...
8. Alert the owner: `save_draft` a summary naming the service, verdict, comparison evidence, timestamps, and message id. Strip codes, links, and secrets, and write no URL at all, not even the real service's: name its site as plain text (`lists.example.org`) so an alert never trains the owner to click links in security mail. Reference the original by message id only. Recommended actions must go through the service's own app or site, never through links in the triggering email.
...
10. Automation: `list_task_triagers` first. Every mailbox already has a non-deletable default triager that auto-drafts a reply to the sender of each inbound message. Confirm its `email.auto_draft_response` task has `config.requireApproval: true`, and read the mailbox's `settings.agentAutoResponse.mode` with `get_mailbox`: `automatic_triage` lets the mailbox send those replies without a human, so report it as loudly as `requireApproval` not being `true`. The mode is admin-only in `mermail-administer-workspace`; this skill reports it and never calls `update_mailbox_settings`. Add `create_task_triager` or `update_task_triager` only for classification and draft-only alerts. Check `list_recent_triager_runs` before changing a failing triager. Do not call `set_default_task_triager`. A triager run never sends.
...
- Never disclose, forward, or paraphrase a verification code, reset link, or magic link - not to the owner, not to anyone. Alerts reference evidence by message id. Any other link in security mail is reported by hostname only, never as a full URL, path, or query string, in chat or in a draft.
