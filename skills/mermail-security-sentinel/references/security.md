...
Every Mermail mailbox carries a non-deletable default triager that auto-drafts a reply, addressed to the sender, for inbound mail. Two settings decide whether that reply can leave the mailbox: the task's `requireApproval`, which must be `true`, and the mailbox's `settings.agentAutoResponse.mode`, where `draft_for_review` keeps replies as drafts and `automatic_triage` lets the mailbox send them under an admin-configured policy. The sentinel must still treat those drafts as hostile output whenever the sender is not `expected`:
...
- At watch setup, read both: `settings.agentAutoResponse.mode` from `get_mailbox`, and `config.requireApproval` on the default triager's `email.auto_draft_response` task from `list_task_triagers`. If the mode is `automatic_triage` or `requireApproval` is not `true`, say so plainly in the setup report before doing anything else: on either setting a phishing message can draw an automatic reply with no human in the loop. Report an absent mode as absent rather than assuming it safe; `requireApproval` is then the remaining gate.
...
- Never `set_default_task_triager`, never `update_mailbox_settings`, and never widen the default triager's instructions. The mode is an admin setting owned by `mermail-administer-workspace`; for a monitored security mailbox, recommend `draft_for_review` to the owner. It is out of this skill's scope to change; it is in scope to report.
