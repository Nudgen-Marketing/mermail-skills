...
Tell a **stated block** apart from an **engine defect**. A `403`, a CAPTCHA, or a challenge page is the vendor speaking; stop. A page that wipes itself under one engine and renders cleanly under another, with no challenge anywhere, is a rendering defect, not a preference — record both results, name the engine that renders, and drive only that one. The probe echoes `engine` for exactly this reason.

Verified example: `https://console.mermail.app/auth` reproduces this signature identically under bundled Chromium, real Chrome, and real Edge (`Uncaught TypeError: Cannot read properties of undefined (reading 'parentNode')`, `document.body === null`, blank capture), in both headless and headful runs — and renders normally under Playwright **Firefox** (three of three runs: "Welcome back", one **Continue with Google** button, Terms and Privacy links). So the crash is a Chromium-family defect, not a block. It changes nothing for this skill: the rendered page's only control is a Google sign-in, which the probe reports as `third-party-login` — a human step. Mermail's own console is therefore a human-only signup either way, which is consistent with the rest of this skill: Mermail supplies the mailbox and the wallet, and a person opens the account.

## Probe before you drive

[probe-signup-origin.mjs](../scripts/probe-signup-origin.mjs) runs the detection above as one read-only Playwright pass — navigate, settle, inspect — and prints a JSON verdict: `renderable`, `blocked_hydration_wipe`, `origin_drift`, or `http_error`. It never fills a field or clicks a control. Run it on the frozen origin before the signup leg; a non-`renderable` verdict is the `blocked` report, ready to paste. On a `renderable` page it also inventories the form and lists `humanSteps` — `password`, `card`, `captcha`, `consent`, `third-party-login` — so the handoff can be named *before* the first keystroke instead of discovered mid-form. `PROBE_BROWSER=firefox` (or `webkit`) reruns it on another engine; the report carries `engine` so the record states what was tried. Verified on live pages: GitHub's signup answers automation with HTTP 403 (`http_error` — a block, reported as such, never worked around); Apify's signup renders one email field and reports `consent`.
