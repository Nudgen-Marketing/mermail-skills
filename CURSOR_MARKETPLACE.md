...
| Longer description | Mermail packages 16 Agent Skills plus a hosted Streamable HTTP MCP server (`https://console.mermail.app/mcp`, Official Registry id `app.mermail/mcp`). Install the plugin, authenticate with Mermail OAuth, and use agent inboxes, verification mail, inbox management, compose/send, workspace admin, task triage, mailbox-agent chat, Composio integrations, scheduling, GTM, support, reward-flow, and x402/Agent Wallet workflows. Destructive tools require MCP confirmation tokens. |
