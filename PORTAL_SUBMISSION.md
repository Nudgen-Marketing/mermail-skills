...
This file is the working checklist for Phase 3. Repo packaging is done in **1.3.1**; you still must complete OpenAI identity + portal submit (cannot be automated without your org login).
...
Output: `dist/mermail-skills-openai.zip` — **skill roots only** at ZIP top level (`mermail/`, `mermail-mcp/`, … each with `SKILL.md`). No `LICENSE`, `.codex-plugin`, or wrapping `skills/` folder (portal rejects those).
...
2. Copy **connector id** → create `.app.json` from `.app.json.example` → set `"apps": "./.app.json"` in `.codex-plugin/plugin.json` → bump patch version → push.
